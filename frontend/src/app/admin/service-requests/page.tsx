...
export default function AdminServiceRequestsPage() {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      toast.error(`Failed to load service requests: ${errorMessage(error)}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveDetails = async () => {
    ...
    try {
      ...
      fetchRequests();
      setShowModal(false);
      toast.success('Notes & price saved');
    } catch (error) {
  ...
  const handleConvertToOrder = async () => {
    ...
    try {
      ...
      setShowModal(false);
      fetchRequests();
      toast.success('Service request converted to order');
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast.success('Service request moved to trash');
    } catch (error) {
  ...
  const downloadAllImages = async (imageString: string, requestId: string) => {
    ...
    for (let i = 0; i < validUrls.length; i++) {
      ...
      const filename = `service-image-${requestId}-${i + 1}.jpg`;
        ...
        <div class="section">
          ...
          <div class="grid">
            ...
            <div class="field">
              ...
              <div class="value" style="color: #1e3a5f; font-size: 18px; font-weight: bold;">৳${request.price!.toFixed(2)}</div>
            </div>
            ` : ''}
            ${request.admin_notes ? `
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[600px]">
                <thead className="bg-gray-50 border-b">
                  <tr>
                    <th className="text-left px-3 py-2 text-xs font-medium text-gray-500 uppercase">Service</th>
